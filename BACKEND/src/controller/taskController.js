...
exports.getTaskByProject = async (req, res) => {
  ...
  try {
    ...
    // Pour chaque tâche, récupérer les utilisateurs assignés
    const tasksWithAssignedUsers = await Promise.all(
      taskResult.rows.map(async (task) => {
        ...
        return {
          ...task,
          assigned_to: assignedUsersResult.rows, // Ajouter les utilisateurs assignés
        };
      })
    );

    // Renvoyer les tâches avec les utilisateurs assignés
    res.status(200).json(tasksWithAssignedUsers);
  } catch (err) {
    console.error(err);
    res.status(500).send("Server error");
  }
...
exports.updateTask = async (req, res) => {
  const { id } = req.params;
  const { title, description, status, priority, deadline, assigned_to } =
    req.body;
...
exports.getTaskComments = async (req, res) => {
  ...
  try {
    ...
    // Formater les commentaires pour inclure les informations de l'utilisateur
